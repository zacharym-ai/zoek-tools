...
exports.handler = async function(event, context) {
  ...
  // ── 3. Forward to Compass ────────────────────────────────────────
  // Compass runs the pipeline: it creates the follow-up tasks, routes the
  // lead to a closer, and reports which tool actually produced business.
  // Wrapped so a Compass outage can never break tool capture — this is a
  // copy, not the system of record.
  if (process.env.COMPASS_URL) {
    try {
      const compassRes = await fetch(`${process.env.COMPASS_URL.replace(/\/$/, '')}/api/magnet`, {
        method: 'POST',
        ...
        body: JSON.stringify({
          ...
          overall_score,
          weak_areas,
          location, industry, extra,
        }),
      });
      console.log('Compass forward:', compassRes.status);
    } catch (e) {
      console.error('Compass forward failed (non-fatal):', e.message);
    }
  }
